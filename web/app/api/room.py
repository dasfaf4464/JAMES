...
def init_socketio(socketio):
    @socketio.on('connect')
    def handle_connect():
        session_code = request.args.get('session_code')  # 쿼리스트링에서 세션코드 추출

        if session_code:
            join_room(session_code)  # room 참여
            print(f"{request.sid} joined room: {session_code}")
            emit('system_message', f"자동으로 세션 {session_code} 방에 입장하였습니다.", room=request.sid)
        else:
